...
<h1 class="text-center my-4"><?= isset($user) && $userRole === 'admin' ? 'Tous les emprunts' : 'Vos emprunts' ?></h1>
...
<?php if (!empty($livresEmpruntes)): ?>
    <div class="container">
        ...
        <div class="text-center">
            <a href="index.php?p=livres" class="btn btn-primary"> <?= isset($user) && $user["role"] === 'admin' ? 'Voir d\'autres livres' : 'Emprunter un autre livre' ?></a>
        </div>
    </div>
<?php else: ?>
    <!-- Si aucun livre n'est emprunté -->
    <div class="alert alert-warning text-center" role="alert">
        <?= isset($user) && $user["role"] === 'admin' ? 'Aucun utilisateur n\'a emprunté de livre' : 'Vous n\'avez actuellement aucun emprunt.' ?>
    </div>
    <!-- Bouton pour emprunter un livre -->
    <div class="text-center">
        <a href="index.php?p=livres" class="btn btn-primary"><?= isset($user) && $user["role"] === 'admin' ? 'Voir d\'autres livres' : 'Emprunter un autre livre' ?></a>
